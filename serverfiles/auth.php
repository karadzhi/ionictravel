...
mysql_connect("localhost","karadzhi_boot","test123") or die("could not connect server"); //local host - server
mysql_select_db("karadzhi_bootstrap") or die("could not connect database");
...
if(isset($_POST['signup']))
{
	...
	if($login!=0) //if is not equal to 0, generate message for the user when u reg
	{
		echo "exist";
	}
	else
	{
		$date=date("d-m-y h:i:s"); // CREATE
